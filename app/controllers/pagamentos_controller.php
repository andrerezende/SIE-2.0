...
class PagamentosController extends AppController {
	...
	public function admin_delete($id = null) {
		...
		if ($this->Pagamento->delete($id)) {
			$this->Session->setFlash(__('Pagamento deleted', true), 'flash');
			$this->redirect(array('action'=>'index'));
		}
		$this->Session->setFlash(__('Pagamento was not deleted', true), 'flash');
		$this->redirect(array('action' => 'index'));
	}
...
//			debug($this->data);exit;
			...
			if ($count == 0) {
				$this->Session->setFlash('Arquivo importado com sucesso', 'flash');
			} else {
				$this->Session->setFlash('Os seguintes registros não foram importados:' . $error . '<br />Total de registros não importados: ' . $count, 'flash');
			}
			$this->redirect(array('action' => 'index'));
		}
	}
