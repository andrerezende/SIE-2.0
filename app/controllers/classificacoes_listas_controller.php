<?php
/**
 * ClassificacoesListasController
 *
 * PHP version 5
 *
 * @author      Vitor Pacheco          <vitor.pacheco@ifbaiano.edu.br>
 * @package     Controller
 * @property    ClassificacoesLista    $ClassificacoesLista
 */
class ClassificacoesListasController extends AppController {
	...
	public function admin_view($id = null) {
		if (!$id) {
			$this->Session->setFlash(__('Invalid classificacoes lista', true), 'flash');
	...
	public function admin_add() {
		if (!empty($this->data)) {
			...
			if ($this->ClassificacoesLista->save($this->data)) {
				$this->Session->setFlash(__('The classificacoes lista has been saved', true), 'flash');
				$this->redirect(array('action' => 'index'));
			} else {
				$this->Session->setFlash(__('The classificacoes lista could not be saved. Please, try again.', true), 'flash');
			}
		}
	...
	public function admin_delete($id = null) {
		if (!$id) {
			$this->Session->setFlash(__('Invalid id for classificacoes lista', true), 'flash');
			$this->redirect(array('action'=>'index'));
		}
		if ($this->ClassificacoesLista->delete($id)) {
			$this->Session->setFlash(__('Classificacoes lista deleted', true), 'flash');
			$this->redirect(array('action'=>'index'));
		}
		$this->Session->setFlash(__('Classificacoes lista was not deleted', true), 'flash');
		$this->redirect(array('action' => 'index'));
	}
