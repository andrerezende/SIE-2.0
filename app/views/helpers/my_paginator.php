 ...
 */
	function __pagingLink($which, $title = null, $options = array(), $disabledTitle = null, $disabledOptions = array()) {
		...
		if ($this->{$check}($model)) {
			...
			return $this->Html->tag($tag, $this->link($title, '#', array('escape' => $escape)), array_merge($options, compact('escape', 'class')));
		}
	}
 ...
 */
	function numbers($options = array()) {
		...
		$defaults = array(
			...
			'modulus' => '8', 'separator' => ' | ', 'first' => null, 'last' => null, 'current' => 'current',
		);
		$options += $defaults;
		...
		unset($options['tag'], $options['before'], $options['after'], $options['model'],
			$options['modulus'], $options['separator'], $options['first'], $options['last'], $options['current']);
		...
		if ($modulus && $params['pageCount'] > $modulus) {
			...
			for ($i = 1; $i <= $params['pageCount']; $i++) {
				if ($i == $params['page']) {
					$out .= $this->Html->tag($tag, $this->link($i, '#'), array('class' => $current));
				} else {
					$out .= $this->Html->tag($tag, $this->link($i, array('page' => $i), $options));
				}
				if ($i != $params['pageCount']) {
